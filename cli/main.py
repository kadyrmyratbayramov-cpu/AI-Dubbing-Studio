...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="AI Dubbing Studio application runner")
    parser.add_argument("--config", help="Path to a YAML or JSON configuration file")
    parser.add_argument("--environment", default=None, help="Runtime environment name")

    subparsers = parser.add_subparsers(dest="command")
    subparsers.required = False

    subparsers.add_parser("info", help="Print runtime metadata")
    subparsers.add_parser("check", help="Initialize the application and verify setup")
    ...
    web_parser = subparsers.add_parser("serve-web", help="Start the web preview server")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    application = create_application(config_path=args.config, environment=args.environment)
    ...
    if command == "serve-api":
        host = args.host or str(application.runtime_settings.get("host", "127.0.0.1"))
        port = int(args.port or application.runtime_settings.get("api_port", 8000))
        run_api_server(host=host, port=port, application=application)
        return 0

    if command == "serve-web":
        host = args.host or str(application.runtime_settings.get("host", "127.0.0.1"))
        port = int(args.port or application.runtime_settings.get("web_port", 8080))
